<div>
    <x-app-layout>
        <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 leading-tight">Create Ticket</h2></x-slot>
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                    <form wire:submit="save">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-span-2"><label>Title *</label><input wire:model="title" type="text" class="w-full rounded-md border-gray-300" required />@error('title')<span class="text-red-500 text-sm">{{ $message }}</span>@enderror</div>
                            <div class="col-span-2"><label>Description *</label><textarea wire:model="description" rows="4" class="w-full rounded-md border-gray-300" required></textarea>@error('description')<span class="text-red-500 text-sm">{{ $message }}</span>@enderror</div>
                            ...
                            <div><label>Category</label><input wire:model="category" type="text" class="w-full rounded-md border-gray-300" placeholder="e.g., Technical, Billing, General" /></div>
                        </div>
                        <div class="mt-6 flex justify-end space-x-2">
                            <a href="{{ route('tickets.index') }}" class="px-4 py-2 bg-gray-300 rounded-md">Cancel</a>
                            <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Create Ticket</button>
                        </div>
                    </form>
                </div>
            </div>
